...
& java -jar swagger-codegen-cli-2.4.5.jar generate -i https://api.cloudmersive.com/swagger/api/currency -l csharp -o client -c packageconfig.json
...
$csprojpath = Resolve-Path ./client/src/Cloudmersive.APIClient.NET.Currency/Cloudmersive.APIClient.NET.Currency.csproj
$csprojtestpath = Resolve-Path ./client/src/Cloudmersive.APIClient.NET.Currency.Test/Cloudmersive.APIClient.NET.Currency.Test.csproj
$nuspecpath = Resolve-Path ./client/src/Cloudmersive.APIClient.NET.Currency/Cloudmersive.APIClient.NET.Currency.nuspec
$slnpath = Resolve-Path ./client/Cloudmersive.APIClient.NET.Currency.sln


(Get-Content $nuspecpath).replace('<title>Swagger Library</title>', "<title>Cloudmersive Currency API Client</title><licenseUrl>https://www.apache.org/licenses/LICENSE-2.0.txt</licenseUrl>") | Set-Content $nuspecpath
...
(Get-Content $nuspecpath).replace('<description>A library generated from a Swagger doc</description>', "<description>The currency APIs help you retrieve exchange rates and convert prices between currencies easily.</description>") | Set-Content $nuspecpath
...
(Get-Content $nuspecpath).replace('<dependencies>', "<projectUrl>https://cloudmersive.com/currency-api</projectUrl><dependencies>") | Set-Content $nuspecpath
...
(Get-Content '.\client\src\Cloudmersive.APIClient.NET.Currency\Client\ApiClient.cs').replace('request.AddFile(param.Value.Name, param.Value.Writer, param.Value.FileName, param.Value.ContentType);', 'request.AddFile(param.Value.Name, param.Value.Writer, param.Value.FileName, param.Value.ContentLength, param.Value.ContentType);') | Set-Content '.\client\src\Cloudmersive.APIClient.NET.Currency\Client\ApiClient.cs'
...
& C:\CodeSigning\sign.ps1 ./client/src/Cloudmersive.ApiClient.NET.Currency/bin/Debug/Cloudmersive.APIClient.NET.Currency.dll
